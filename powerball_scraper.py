...
class PowerballScraper:
    ...
    def format_date_iso(self, date_str):
        ...
        try:
            ...
            # Reemplazar abreviatura de mes por nombre completo (case-insensitive)
    ...
    def scrape_results(self):
        ...
        try:
            ...
            # ✅ FIX: Leer la fecha de id="numbers" (Números Ganadores actuales)
            # ANTES usaba id="winners" que contiene la fecha del sorteo ANTERIOR
            date_element = None
            numbers_section = soup.find('div', class_='col', id='numbers')
            if numbers_section:
                date_element = numbers_section.find('h5', class_='card-title')
                logging.info(f"Fecha encontrada en sección #numbers")
            
            # Fallback: buscar en toda la página si no se encontró en #numbers
            if not date_element:
                date_element = soup.find('h5', class_='card-title')
                logging.info(f"Fecha encontrada con fallback general")
            ...
            # Extraer Premio Estimado (Jackpot)
...
def main():
    ...
    if results.get('_success'):
        ...
        # Leer JSON actual solo para comparar fecha anterior
        json_actual = scraper.leer_json_actual()
        fecha_anterior = None
        if json_actual and 'sorteo' in json_actual:
            fecha_anterior = json_actual['sorteo'].get('fecha')
        
        # Validar fecha scrapeada
        fecha_scrapeda = sorteo.get('fecha')
        
        if fecha_scrapeda:
            # Usar siempre la fecha más reciente entre scrapeada y calculada
            fecha_calculada = scraper.calcular_fecha_ultimo_sorteo()
            if fecha_calculada > fecha_scrapeda:
                logging.warning(f"⚠️ Fecha scrapeada ({fecha_scrapeda}) menor que calculada ({fecha_calculada}), usando calculada")
                sorteo['fecha'] = fecha_calculada
            else:
                logging.info(f"✓ Fecha scrapeada correcta: {fecha_scrapeda}")
        else:
            fecha_calculada = scraper.calcular_fecha_ultimo_sorteo()
        ...
        if fecha_anterior:
            logging.info(f"Fecha anterior: {fecha_anterior} → Nueva: {sorteo['fecha']}")
